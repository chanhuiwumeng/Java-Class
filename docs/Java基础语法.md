...
> - 代码末尾加分号
> - **类名**：对于所有的类来说，类名的首字母应该大写。如果类名由若干单词组成，那么每个单词的首字母应该大写，例如 **MyFirstJavaClass**   大驼峰命名法
> - **方法名**：所有的方法名都应该以小写字母开头。如果方法名含有若干单词，第一个单词首字母小写，后面的每个单词首字母大写。  小驼峰命名法。
...
> 方法(函数):
>  	前面学习数组的排序，每次一种排序完成以后我们都要去遍历数组
>  	我们能不能将遍历的代码进行一个封装，方便我们去进行复用。
>  	在将数组的排序，每一种排序的代码在封装到一起，进行复用。
>  方法: 
>  	方法就是对一段代码(功能)的封装。提高代码的复用性。	
>  	1. 方法的声明:
>  		访问修饰符  修饰符  返回值类型  方法名称(参数列表){
>  			方法体;
>  		}
>  	javaSe中 main方法虚拟机的主入口，也是主要的核心的方法。
>  		main方法和唯一的注入口。其他的方法名字不能和他重复。
>  	访问修饰符: (面向对象中讲  封装)
>  		public  公开的
>  		protected  受保护的
>  		default    默认的
...
>  			没有参数的方法
>  		b. 有返回值的方法
>  				有返回值的方法 方法执行完成以后返回的是一个方法返回值类型的值。
>  				使用 return 返回方法的返回值。
>  				返回值的数据类型 必须和方法声明的返回值数据类型一致。
>  				我们要去接收返回值方法的返回值，在进行其他的运算。
>  			没有返回值的方法  void 进行修饰
>  	3. 方法只有被调用才能被执行。	方法不能自己执行。
>  		方法可以被调用多次。每一次的调用都是独立执行的。
>  		方法是可以互相调用的。但是不能嵌套调用 会出现值栈溢出(内存溢出 java.lang.StackOverflowError)
>  		出现值栈溢出 那肯定就是死循环了。
>  		我们main方法是注入口  所以我们在执行的时候将方法教给main进行调用。
>  	4. 被static 修饰的方法只能调用 被static修饰的方法。
>  	5. 方法的重载:
>  		方法的重载就是: 方法名称相同 方法的参数列表不同(参数的个数不同/参数的顺序不同/参数的数据类型不同)与返回值无关
> 			
> 
